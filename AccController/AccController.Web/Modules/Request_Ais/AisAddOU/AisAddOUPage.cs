...
namespace AccController.Request_Ais.Pages
{
    ...
    using MyController = AccController.Request_Ais.Endpoints.AisAddOUController;
    using MyRow = Entities.AisAddOURow;
    using System;
    ...
    public class AisAddOUController : Controller
    {
        ...
        public ActionResult GetRequestFile(string status)
        {
            ...
            {
                var spsHelper = new SpreedSheetHelper(Server.MapPath("~/Content/templates/export/result/ais/ThemDonViUser.xlsx"));
        ...
        public ActionResult GetResultFromFile(string status)
        {
            ...
            try
            {
                ...
                var spsHelper = new SpreedSheetHelper(Server.MapPath("~/Content/templates/import/result/ais/ThemDonViUser.xlsx"));
                    ...
                    var fileRowResponse = this.InTransaction("Default", (uow) =>
                    {
                        ...
                        if (saveFileResponse.EntityId.HasValue)
                        {
                            foreach (var item in list.Entities)
                            {
                                ...
                                var saveresponse = new AisAddOURepository().UpdateResult(uow, new SaveRequest<MyRow>
                                {
                                    Entity = item
                                });
