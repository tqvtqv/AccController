...
namespace AccController.Ais.Endpoints
{
    ...
    using MyRepository = Repositories.AisUserChangeInfoRepository;
    using MyRow = Entities.AisUserChangeInfoRow;

    [RoutePrefix("Services/Ais/AisUserChangeInfo"), Route("{action}")]
    [ConnectionKey("Default"), ServiceAuthorize("AisUser")]
    public class AisUserChangeInfoController : ServiceEndpoint
    {
        [HttpPost]
        ...
        public Result<SaveResponse> updateuserchangeinfo(SaveRequest<MyRow> para1)
        {
            var str = para1;
